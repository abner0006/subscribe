...
# 输出到脚本所在目录（assets/simple/ 目录）
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))  # 当前脚本所在目录
OUTPUT_FILE = os.path.join(SCRIPT_DIR, "simple.txt")    # 结果文件放在同目录

# 数据源和依赖文件路径
RAW_SOURCES = os.path.join(SCRIPT_DIR, "raw_sources.txt")  # 原始数据源
RENAME_FILE = os.path.join(SCRIPT_DIR, "simple_rename.txt") # 容错文件
